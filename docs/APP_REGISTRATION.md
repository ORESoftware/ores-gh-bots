...
ORES uses four mandatory review-runtime identities, a dedicated fleet merge identity, and a separately restricted Actions dispatcher. The Apps remain independent in production so a single credential cannot publish both provider decisions and the aggregate gate.
...
| Merge reaper | Public, not Marketplace-listed | Every managed organization | Checks: read; Contents: write; Metadata: read; Pull requests: write; Commit statuses: read | None |
| Actions dispatcher | Private | Only `ORESoftware/ores-gh-bots` | Actions: write; Metadata: read | None |

A private GitHub App can only be installed on the account that owns it. The five fleet Apps are therefore public so they can be installed across the separate ORES organizations. Public visibility does **not** list an App in GitHub Marketplace; do not submit these Apps for Marketplace publication.
...
2. Create a registration form and a separate private callback-state record for each role. All six forms require the deployed HTTPS base URL for the one-time manifest conversion callback; the orchestrator also derives its webhook URL from that base:
   ...
   just app-form reaper ORESoftware https://bots.example.internal user /tmp/reaper.html /tmp/reaper.state.json
...
4. Within one hour, read the callback values into environment variables without placing either value in shell history, then exchange them against the private state record:
   ...
   Repeat for `openai`, `claude`, `gate`, `reaper`, and `actions` with the corresponding state file. The converter rejects callback-state mismatches and expired state records before contacting GitHub. It also rejects `--code` and `--state`, supports mode-`0600` `--code-file` and `--callback-state-file` inputs for non-interactive operators, and deletes the state record only after a successful conversion.
...
Copy `config/installations.example.json` to the reviewed inventory location used by deployment automation and replace all `null` and `replace-with-*` values. Install the five public Apps on every target organization. During canarying, selected repositories are acceptable; before fleet enforcement, every governed repository must appear in the inventory or be covered by an `all` installation.
