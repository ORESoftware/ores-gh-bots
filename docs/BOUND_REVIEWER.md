...
The deployed orchestrator runs the reviewer reconciler at `RECONCILE_INTERVAL_MS` only when `REVIEWER_APPROVAL_MODE=requested-gate-success`. Late review requests are discovered from GitHub live state on a reconciliation pass; the scheduler does not invent or trust a separate email/webhook authorization path.
