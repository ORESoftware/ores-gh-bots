...
function approvalBody(candidate) {
  return [
    'The exact pull-request head passed the configured aggregate ORES gate.',
    '',
  ...
  async runOnce() {
    ...
    try {
      ...
      for (const candidate of plan.candidates) {
        ...
        try {
          const gate = await findSuccessfulBoundReviewerGate({
            client: this.client,
            token: this.config.reviewer.token,
...
export function startReviewerReconciler(reconciler, intervalMs, signal) {
  if (!Number.isSafeInteger(intervalMs) || intervalMs < 60_000) {
    throw new Error('reviewer reconciliation interval must be at least 60000ms');
  }
  const run = () => reconciler.runOnce().catch((error) => {
    reconciler.logger.error('bound reviewer reconciliation failed', {
