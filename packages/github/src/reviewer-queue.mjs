...
import {
  ...
  listBoundReviewerPullRequestReviews,
} from './bound-reviewer.mjs';
import { getCollaboratorPermission } from './pull-requests.mjs';
import {
  addCandidate,
...
export async function assertReviewerIdentity(client, token, expectedLogin) {
  return assertBoundReviewerIdentity(client, token, expectedLogin);
}

export async function listPullRequestReviews(client, token, owner, repo, prNumber) {
  return listBoundReviewerPullRequestReviews(client, token, owner, repo, prNumber);
}

export async function getReviewerPermission(client, token, owner, repo, reviewer) {
  try {
    const permission = await getCollaboratorPermission(client, token, owner, repo, reviewer);
    return boundedString(permission, 'collaborator permission', 32).toLowerCase() || 'none';
  } catch (error) {
...
export function currentHeadReview(allReviews, reviewerLogin, headSha) {
  return currentBoundReviewerHeadReview(allReviews, reviewerLogin, headSha);
}
...
async function liveCandidate(client, token, reviewer, reference) {
  ...
  if (!SHA.test(String(pullRequest?.head?.sha ?? '').toLowerCase())) {
    throw new Error('live pull request returned an invalid head SHA');
  }
...
export async function buildReviewerQueue({
  client,
  token,
  ...
  for (const hint of parseReviewerHints(hints, reviewer)) {
    ...
    for (const hintId of hint.hintIds) current.hintIds.add(hintId);
  ...
  const selected = [...references.values()]
    .sort((left, right) => (
      ...
      || candidateKey(left.owner, left.repo, left.prNumber).localeCompare(
        candidateKey(right.owner, right.repo, right.prNumber),
      )
    ))
    .slice(0, max);
  ...
  const candidates = await mapLimit(selected, LIVE_FETCH_CONCURRENCY, async (reference) => {
    try {
      return await liveCandidate(client, token, reviewer, reference);
    } catch (error) {
      return Object.freeze({
        repository: `${reference.owner}/${reference.repo}`,
  ...
  candidates.sort((left, right) => (
    (rank[left.disposition] ?? 99) - (rank[right.disposition] ?? 99)
